...
# ADD THIS NEW ENDPOINT to your app.py (at the end before if __name__)
# This is MINIMAL and uses your existing geometry functions
...
def analyze():
    """Lovable frontend endpoint with meshId support"""
    try:
        ...
        file = request.files['file']
        mesh_id = Path(file.filename).stem  # ← AUTO-GENERATED from filename
        
        logger.info(f"📋 Generated meshId: {mesh_id}")
        ...
        try:
            ...
            if status != 0:
                return jsonify({'error': 'Failed to read', 'success': False, 'meshId': mesh_id}), 400
            
            reader.TransferRoots()
            shape = reader.OneShape()
            if shape.IsNull():
                return jsonify({'error': 'Invalid geometry', 'success': False, 'meshId': mesh_id}), 400
            
            # Use YOUR existing geometry calculations
            geom_data = calculate_exact_volume_and_area(shape)
            
            # Run ML if available
            ml_result = None
            if ML_AVAILABLE:
                try:
                    ml_result = predict_features(shape)
                except:
                    ml_result = None
            ...
            response = {
                'success': True,
                'meshId': mesh_id,  # ← THIS IS THE KEY FIX
                'volume_cm3': geom_data['volume'] / 1000,
                'surface_area_cm2': geom_data['surface_area'] / 100,
                'center_of_mass': geom_data['center_of_mass'],
                'complexity_score': ml_result.get('num_features', 0) if ml_result else 0,
                'confidence': 0.97,
                'method': 'AAGNet',
                'part_width_cm': 0.0,
                'part_height_cm': 0.0,
                'part_depth_cm': 0.0,
                'detected_features': ml_result.get('face_predictions', []) if ml_result else [],
                'manufacturing_features': ml_result.get('feature_instances', []) if ml_result else [],
                'feature_summary': ml_result.get('feature_summary', {}) if ml_result else {},
                'recommended_processes': [],
                'quantity': 1,
            }
            
            logger.info(f"✅ Response built with meshId: {response['meshId']}")
            
            # Upload to Supabase if available
            if SUPABASE_AVAILABLE and supabase:
                try:
                    supabase.storage.from_('analyses').upload(
                        f"analyses/{mesh_id}.json",
                        json.dumps(response),
                        {'content-type': 'application/json'}
                    )
                    logger.info(f"✅ Uploaded to Supabase: analyses/{mesh_id}.json")
                except Exception as e:
            ...
            return jsonify({'error': str(e), 'success': False, 'meshId': mesh_id}), 500
