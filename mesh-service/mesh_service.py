...
def calculate_vertex_normals(vertices, indices, sharp_edge_threshold=30.0):
    ...
    For each vertex, average the normals of all adjacent faces,
    weighted by the face area (larger faces contribute more).
    
    This produces smooth shading on curved surfaces while naturally
    preserving sharp edges from the geometry itself.
    
    Args:
        vertices: Flat list of vertex coordinates [x,y,z, x,y,z, ...]
        indices: Triangle indices
        sharp_edge_threshold: Not used (kept for API compatibility)
    ...
    # For each face, calculate face normal weighted by area
    # and add it to each vertex of the face
    for i in range(num_faces):
        ...
        # The length of the cross product is 2x the triangle area
        # So we don't need to normalize - the length provides area weighting
        # Larger faces will contribute more to the vertex normal
        
        # Add this face's contribution to all three vertices
        vertex_normals[i1] += face_normal
        vertex_normals[i2] += face_normal
        vertex_normals[i3] += face_normal
