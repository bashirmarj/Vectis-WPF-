...
import { ChevronDown, ChevronRight, Box, Circle, Square, Triangle, Layers, Zap, Sparkles } from "lucide-react";
...
const FEATURE_CATEGORIES = {
  ...
  pockets: [
    // New taxonomy
    "closed_rectangular_pocket",
    "open_rectangular_pocket",
    "circular_pocket",
    "irregular_pocket",
    "obround_pocket",
    "island_pocket",
  ...
  slots: [
    // New taxonomy
    "through_slot_rectangular",
    "blind_slot_rectangular",
    "open_u_slot",
    "corner_slot",
    "t_slot",
    "dovetail_slot",
  ...
  bosses: ["cylindrical_boss", "rectangular_boss", "irregular_boss", "rib", "lug", "boss"],
  fillets: [
  ...
  chamfers: [
    "equal_distance_chamfer",
    "distance_angle_chamfer",
    "two_distance_chamfer",
    "corner_chamfer",
    "chamfer",
    "45_degree",
    "angled",
...
const FeatureTree: React.FC<FeatureTreeProps> = ({ features, onFeatureSelect, selectedFeature }) => {
  ...
  const [expandedCategories, setExpandedCategories] = React.useState<Set<string>>(new Set(Object.keys(CATEGORY_NAMES)));
  ...
  const numberedFeatures = useMemo(() => {
    ...
    Object.entries(categorizedFeatures).forEach(([category, instances]) => {
      instances.forEach((instance) => {
        ...
        const baseType =
          (instance.subtype && FEATURE_DISPLAY_NAMES[instance.subtype]) ||
          ...
          {numberedFeatures.map(({ instance, displayName, icon: Icon }, idx) => (
            <div
              ...
              className={`flex items-center gap-2 px-3 py-1.5 cursor-pointer transition-colors ${
                selectedFeature === instance ? "bg-primary/20 border-l-2 border-primary" : "hover:bg-accent/50"
              }`}
            >
              <Icon className="w-4 h-4 text-muted-foreground flex-shrink-0" />
              <span className="text-sm font-medium text-foreground flex-1">{displayName}</span>
