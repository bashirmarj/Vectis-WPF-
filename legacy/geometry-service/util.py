...
def center_and_scale_uvgrid(inp: torch.Tensor, return_center_scale=False):
    """
    Center and scale UV-grid features using bounding box
    EXACT copy from References/cad-feature-detection-master/backend/app/occwl/util.py
    """
...
def bounding_box_uvgrid(inp: torch.Tensor):
    """Extract bounding box from UV-grid with visibility masking"""
...
def face_classification_to_label(index):
    """Map feature class index to name (16 MFCAD classes)"""
    face_map = {
        "15": "stock",
    ...
    return face_map.get(str(index), "unknown")
